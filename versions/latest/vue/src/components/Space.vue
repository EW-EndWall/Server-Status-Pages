      ...
      <div class="coming_content">
        ...
        <div class="separator_container">
          <div class="separator line_separator">
            <span v-if="componentData?.centerSvgIconUrl">
              <img :src="componentData.centerSvgIconUrl" alt="logo" />
              <!-- /other/building/img/img.svg -->
            </span>
          </div>
        </div>
        <h2 v-if="componentData?.message" v-html="componentData.message"></h2>
        <h3 v-if="componentData?.copyrightFromTitle">
          copyright from
          <a :href="componentData?.copyrightFromUrl">
            {{ componentData.copyrightFromTitle }}
          </a>
        </h3>
        <hr
          v-if="
            componentData?.copyrightFromTitle && componentData?.poweredByTitle
          "
          class="foter_hr"
        />
        <h4 v-if="componentData?.poweredByTitle">
          powered by
          <a :href="componentData?.poweredByUrl">
            {{ componentData.poweredByTitle }}
          </a>
        </h4>
      </div>
    </section>
